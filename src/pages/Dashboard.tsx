...
const quickStats = [
  ...
  },
  {
    ...
    icon: CheckCircle2,
    color: 'bg-night-100 text-light-400',
  },
  {
...
const priorityColors = {
  ...
  low: 'bg-night-100 text-light-500',
};
        ...
        {quickStats.map((stat) => (
          <Card key={stat.label} className="hover:shadow-md transition-shadow">
            <CardContent>
              ...
              <p className="text-2xl font-bold text-light-300">{stat.value}</p>
              <p className="text-sm text-light-500 mt-1">{stat.label}</p>
              <p className="text-xs text-neon mt-2">{stat.change}</p>
            </CardContent>
          </Card>
        ))}
      </div>
          ...
          <CardContent>
            <div className="space-y-4">
              {recentActivity.map((activity) => (
                <div
                  key={activity.id}
                  className="flex items-center gap-4 p-3 rounded-lg hover:bg-night-100 transition-colors"
                >
                  <div className="p-2 bg-night-100 rounded-lg">
                    <activity.icon className="w-4 h-4 text-light-500" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-light-300 truncate">
                      {activity.action}
                    </p>
                    <p className="text-xs text-light-500">{activity.time}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <CardContent>
            <div className="space-y-3">
              {upcomingTasks.map((task) => (
                <div
                  key={task.id}
                  className="p-3 rounded-lg border border-night-100 hover:border-electric/30 transition-colors"
                >
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <p className="text-sm font-medium text-light-300 line-clamp-1">
                      {task.title}
                    </p>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <Link
                  key={item.href}
                  to={item.href}
                  className="flex items-center gap-3 p-4 rounded-xl bg-night-100 hover:bg-night-50 border border-night-100 hover:border-electric/30 transition-colors"
                >
                  <item.icon className="w-5 h-5 text-electric" />
                  <span className="font-medium text-light-300">{item.label}</span>
                </Link>
              ))}
            </div>
          </CardContent>
          ...
          <CardContent>
            <dl className="space-y-4">
              <div className="flex justify-between py-2 border-b border-night-100">
                <dt className="text-light-500">Nom</dt>
                <dd className="font-medium text-light-300">
                  {profile?.full_name || 'Non renseigne'}
                </dd>
              </div>
              <div className="flex justify-between py-2 border-b border-night-100">
                <dt className="text-light-500">Email</dt>
                <dd className="font-medium text-light-300">{user?.email}</dd>
              </div>
              <div className="flex justify-between py-2 border-b border-night-100">
                <dt className="text-light-500">Membre depuis</dt>
                <dd className="font-medium text-light-300">{createdAt}</dd>
              </div>
              <div className="flex justify-between py-2">
                <dt className="text-light-500">Statut</dt>
                <dd className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium bg-neon/10 text-neon">
